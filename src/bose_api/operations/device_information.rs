...
pub fn parse_device_information(response: &[u8]) -> Result<DeviceInformationInfo, BoseError> {
    ...
    let address_bytes = &response[0..6];
    ...
    let status = DeviceStatus::from(response[6]);
    let name = String::from_utf8_lossy(&response[9..]).to_string();
...
mod tests {
    ...
    fn test_device_information_parsing() {
        let sample_response = vec![
            0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x01, 0x00, 0x42, 0x6f, 0x73, 0x65, 0x20, 0x51,
            0x43, 0x33, 0x35,
        ];
        let device_information_info = parse_device_information(&sample_response).unwrap();
        assert_eq!(
    ...
    fn test_device_information_parsing_invalid() {
        ...
        let result = parse_device_information(&sample_response);
